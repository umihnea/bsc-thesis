...
Its rather non-descriptive name comes from the action-value function $Q$ (which itself presumably stands for ``quality'').

Q-learning learns a (deterministic) optimal policy from an $\varepsilon$-greedy exploratory policy.
Q-learning achieves coverage, since a greedy policy is ``included'' into its $\varepsilon$-greedy couterpart.
