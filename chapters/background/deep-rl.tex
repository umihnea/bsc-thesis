...
\begin{equation}
    \pi(a \given s) \approx \hat{\pi}(a \given s; \theta)
\end{equation}
...
However, in the approximated case, deterministic policies can fail due to partial observability of the environment (since we are relying on \emph{features} which by definition attempt to extract relevant but partial information).
...
A deterministic agent could either decide to go East or West on both grey tiles.
Either choice results in a possibility of getting stuck (the policy creates a loop).
A stochastic policy assigning $0.5$ to both East and West on a grey square would perform better in this case.
